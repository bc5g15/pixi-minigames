...
function drawDisplay(container, codeLength, size)
{
    ...
    const BTs = ["Shampoo", "Conditioner", "Soap", "Foam Bath"];
    ...
    for(let i = 0; i<codeLength; i++)
    {
        ...
        const backtext = new PIXI.Text(BTs[i], {fill: 0xCCCCCC, fontSize:(size/BTs[i].length)});
        backtext.anchor.set(0.5);
        backtext.x = cx;
        backtext.y = 0;
        tempC.addChild(backtext);

        const txt = new PIXI.Text("", {fill: 0xFFFFFF, fontSize:size});
        txt.anchor.set(0.5);
        txt.x = cx; txt.y = 0;
        tempC.addChild(txt);
        output.push({graph: g, text: txt, backtext: backtext});
    }
